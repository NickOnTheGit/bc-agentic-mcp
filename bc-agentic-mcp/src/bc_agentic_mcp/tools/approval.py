...
_APPROVAL_TOKEN_RE = re.compile(r"(?im)^\*\*Approval token:\*\*\s*(\S+)")
_STATUS_RE = re.compile(r"(?im)^\*\*Status:\*\*\s*([A-Za-z_]+)")


def _resolve_request_approval_artifact(root: Path, spec_name: str, phase: str, artifact_path: str) -> str:
    """Resolve the artifact path that must be shown to the human reviewer."""
    if _LEGACY_TO_GATE.get(phase, phase) != "plan":
        return artifact_path

    specs_dir = specs_root(root) / spec_name
    if (specs_dir / "feature_plan.json").exists():
        return str(specs_dir / "FEATURE-REVIEW.md")
    return str(specs_dir / "REVIEW.md")


def _approval_token(text: str) -> str:
    match = _APPROVAL_TOKEN_RE.search(text)
    return match.group(1) if match else ""


def _verify_pending_approval(
    root: Path,
    spec_name: str,
    phase: str,
    approval_path: Path,
    ...
    try:
        text = approval_path.read_text(encoding="utf-8", errors="replace")
    except OSError as exc:
        return None, f"approval artifact unreadable: {exc}"
    ...
    if payload is None:
        return None, "approval artifact is unsigned or has been modified"
    status = _STATUS_RE.search(text)
    if not status or status.group(1).lower() != "pending":
        return None, "approval artifact status was modified before the decision"
    artifact_path = str(payload.get("artifact_path") or "")
    if artifact_path:
        artifact = Path(artifact_path)
        if not artifact.is_file():
            return None, "the approved artifact no longer exists"
        try:
            if security.file_digest(artifact) != payload.get("artifact_sha256"):
                return None, "the approved artifact changed after the approval request"
        except OSError as exc:
            return None, f"the approved artifact could not be hashed: {exc}"
    return payload, None
...
async def handle_request_approval(
    ...
    root = Path(project_root).resolve()
    artifact_path = _resolve_request_approval_artifact(root, spec_name, phase, artifact_path)
    ...
    try:
        artifact_sha256 = security.file_digest(artifact)
    except OSError as exc:
        return {
            "status": "blocked_artifact_unreadable",
            "blocked": True,
            "reason": f"Approval refused: artifact could not be hashed: {exc}",
        }
...
---
Do not edit this file. The human decision must be submitted through bc_submit_decision;
the signed token binds the decision to this exact artifact.
"""
    ...
    return {
        ...
        "status": "pending",
        "instructions": "Show the signed review artifact to the human, then call bc_submit_decision.",
        "approval_integrity": "server-signed",
...
async def handle_submit_decision(
    ...
    pending_payload, integrity_error = _verify_pending_approval(
        root, spec_name, phase, approval_path
    )
    if integrity_error:
        return {
            "status": "blocked_approval_integrity",
            "blocked": True,
            "phase": phase,
            "message": f"Approval blocked: {integrity_error}.",
            "next_action": {
                "tool": "bc_request_approval",
                "params_hint": {"spec_name": spec_name, "phase": phase},
            },
        }
    ...
    decision_payload = dict(pending_payload or {})
    decision_payload["status"] = decision
    decision_payload["decided_at"] = datetime.now(timezone.utc).isoformat()
    decision_token = security.issue("approval", decision_payload)
    security.register_approval_token(decision_token)
    content = _APPROVAL_TOKEN_RE.sub(
        f"**Approval token:** {decision_token}", content, count=1
    )
    ...
    if decision == "approve" and phase == "plan" and is_feature_folder:
        try:
            ...
            for child in feature_children_specs(root, spec_name):
                ...
                feature_review_path = specs_root(root) / spec_name / "FEATURE-REVIEW.md"
                feature_review_sha = (
                    security.file_digest(feature_review_path)
                    if feature_review_path.is_file() else ""
                )
                child_submitted = datetime.now(timezone.utc).isoformat()
                child_token = security.issue_approval(
                    project_root=root,
                    spec_name=item_spec,
                    phase="plan",
                    status="approve",
                    artifact_path=str(feature_review_path) if feature_review_sha else "",
                    artifact_sha256=feature_review_sha,
                    summary=f"Feature-wide approval via {spec_name}",
                    idempotency_key=f"feature:{spec_name}:child:{item_spec}",
                    recorded_at=child_submitted,
                )
                child_path.write_text(
                    f"# Approval: {item_spec} — Plan Phase\n\n"
                    f"**Status:** approve\n"
                    f"**Submitted:** {child_submitted}\n"
                    f"**Artifact:** ../{spec_name}/FEATURE-REVIEW.md\n\n"
                    f"**Approval token:** {child_token}\n\n"
